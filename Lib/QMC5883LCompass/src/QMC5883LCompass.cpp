...
void QMC5883LCompass::init(){
	Wire.begin();
	_writeReg(0x0A,0x0);		// enable DRDY Interrupt, rollover disable
	_writeReg(0x0B,0x01);		// set/reset period register to 1
	setMode(0x01,0x00,0x10,0X00);
	_vRaw[0] = 0;
	_vRaw[1] = 0;
	_vRaw[2] = 0;
	_flag = false;
	//attachInterrupt(18, drdyISR, HIGH);
}
...
bool QMC5883LCompass::getFlag()
{
	return _flag;
}

void QMC5883LCompass::setFlag(bool flag)
{
	_flag = flag;
}
...
/**
	DRDY Line Interrupt Service Routine
**/
...
byte QMC5883LCompass::readStatus()
{
	byte Result = 0;
...
int QMC5883LCompass::getXRaw(){
	return _vRaw[0];
}

int QMC5883LCompass::getYRaw(){
	return _vRaw[1];
}

int QMC5883LCompass::getZRaw(){
	return _vRaw[2];
}
