...
namespace RecordData
{
    public partial class frmMain : Form
    {
        ...
        private void btnSaveReadings_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                if (lstReadings.Items.Count == 0)
                    MessageBox.Show("No data to be saved");
                else
                    using (StreamWriter sw = new StreamWriter(fileName))
                    {
                        foreach(string line in lstReadings.Items)
                            sw.WriteLine(line);
                    }
            }
            catch(Exception x)
        ...
        {
            for (int i = lstReadings.SelectedItems.Count - 1; i >= 0; i--)
            {
                lstReadings.Items.RemoveAt(lstReadings.SelectedIndices[i]);
            }
        }
